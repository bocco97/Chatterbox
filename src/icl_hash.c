...
static int string_compare(void* a, void* b) 
{
    ...
    return (strncmp( (char*)a, (char*)b , strlen((char*)a)) == 0);
}



void LockHash(pthread_mutex_t *qlock, int i)             { pthread_mutex_lock  (&qlock[i]); }
void UnlockHash(pthread_mutex_t *qlock, int i)           { pthread_mutex_unlock(&qlock[i]); }
/**
...
icl_hash_create(unsigned int (*hash_function)(void*), int (*hash_key_compare)(void*, void*) )
{
    ...
    ht->nentries = 0;
    ht->buckets = (icl_entry_t**)malloc(N_BUCKETS* sizeof(icl_entry_t*));
    ...
    for (int j=0;j<N_SECTIONS;j++) pthread_mutex_init(&ht->qlock[j],NULL);
...
icl_hash_find(icl_hash_t *ht, void* key)
{
    ...
    int index=((* ht->hash_function)(key) % ht->nbuckets)%N_SECTIONS;

    LockHash(ht->qlock,index);

    if(!ht || !key) {  UnlockHash(ht->qlock,index); return NULL;}
...
icl_hash_insert(icl_hash_t *ht, void* key, void *data)
{
    ...
    curr->key=(char*)malloc((MAX_NAME_LENGTH+1)*sizeof(char));
    strncpy(curr->key,(char*)key,MAX_NAME_LENGTH+1);
    curr->histSize=0;
    //curr->key = key;
    curr->data = NULL;                                            //non mi serve settare data perchè aggiungendo un utente nell ht setto solo key(nickname), non data(msgPendenti)
...
int icl_hash_delete(icl_hash_t *ht, void* key, void (*free_key)(void*), void (*free_data)(void*))
{
    ...
    if(!ht || !key){  UnlockHash(ht->qlock,index); return -1;}
    hash_val = (* ht->hash_function)(key) % ht->nbuckets;
    ...
    for (curr=ht->buckets[hash_val]; curr != NULL; )  {
        if ( ht->hash_key_compare(curr->key, key)) {
            ...
            if(curr->data) delete_msg_Queue(curr->data);
